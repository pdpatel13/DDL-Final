...
class Diffusion(object):
    def __init__(self, args, config, device=None):
        ...
        alphas = (1.0 - self.betas).to(self.device)
    ...
    def sg_train(self):
        ...
        if self.args.dataset=='LDFDCT':
            ...
            # CHEXPERT for brain image translation
            dataset = CheXpert(self.config.data.train_dataroot, self.config.data.image_size, split='train')
        ...
        train_loader = data.DataLoader(
            dataset,
            batch_size=config.training.batch_size // (self.world_size if args.distributed else 1),
            shuffle=shuffle,
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, x in enumerate(train_loader):
                ...
                if self.rank == 0 and step % self.config.training.snapshot_freq == 0 or step == 1:
                    states = [
    ...
    def sg_sample_fid(self, model):
        ...
        # Dataset loading part remains the same...
        if self.args.dataset=='CheXpert':
            from datasets.chexpert import CheXpert
            sample_dataset = CheXpert(self.config.data.sample_dataroot, self.config.data.image_size, split='calculate')
            print('Start sampling model on CheXpert dataset.')
        ...
        with torch.no_grad():
            ...
            for batch_idx, sample in tqdm.tqdm(enumerate(sample_loader), desc="Generating image samples for FID evaluation."):
                ...
                # Transform and prepare images
                x = inverse_data_transform(config, x)
                x_gt = inverse_data_transform(config, x_gt)

                # Save tensors for image saving
                x_tensor = x.clone()
                x_gt_tensor = x_gt.clone()
                ...
                SSIM = 0.0
                for i in range(x.shape[0]):
                    psnr_temp = calculate_psnr(x[i], x_gt[i])  # Removed extra dimensions
                ...
                avg_psnr += PSNR
                avg_ssim += SSIM
                logging.info('Case {}: PSNR {}, SSIM {}, time {}'.format(
                    case_name[0], PSNR_print, SSIM_print, case_time))

                # Save images
                for i in range(n):
                    tvu.save_image(
                        x_tensor[i], 
                        os.path.join(self.args.image_folder, f"{self.ckpt_idx}_{img_id}_pt.png")
                    )
                    tvu.save_image(
                        x_gt_tensor[i], 
                        os.path.join(self.args.image_folder, f"{self.ckpt_idx}_{img_id}_gt.png")
                    )
                    img_id += 1
